...
export const PLANTBOOK_REFERENCE_TABLES = {
  ...
  //
  // Reads the bare table now, NOT a view (Andrew, 2026-09-17b: "make the
  // producer supplier labs table simpler... lab name column with plant
  // location and without AMP number redundancy"). lab_name IS
  // plants.name - trigger-maintained
  // (trg_sync_producer_supplier_lab_name on insert/amp_number change,
  // trg_cascade_plant_name_to_labs when plants.name is corrected later) -
  // rather than a live join, so it reads "Company @ Site" straight off
  // the base table in Studio with no view to remember. This reverses the
  // 2026-09-17 decision to keep it a view "so it can never drift"; the
  // trigger is the same guarantee by a different mechanism (automatic
  // sync instead of read-time join), traded for a table that is simpler
  // to browse directly. `company_name` is gone - it was fully superseded
  // once lab_name became the plant's own name.
  // `label` stays a pure function of its own row, like every other entry
  // here: this table's label/value/aliases are called directly with
  // fixture rows by check_page_plantbook.mjs in a Node vm context with no
  // `state` global, so reaching into another table inside the function
  // (state.ref.plants, tried first for this exact field) throws there -
  // the reason the plant lookup has to live in SQL rather than in this
  // function, whichever mechanism does it.
  //
  // flagged_mismatch: 9 rows where the export's original company name
  // disagreed with plants.name for the same amp_number (7 from the
  // initial reconciliation, 2 more from the 2026-09-17b duplicate-AMP
  // pass - see docs/plantbook-lab-id-reconciliation.md sections 1 and 5).
  // Kept rather than dropped, since neither side is confirmed wrong. The
  // marker in the label makes it "unverified", not "wrong" or "old" -
  // Andrew: H. G. Mays Corp is one of the original 7 and is very much an
  // active company, so the flag can only honestly say the AMP mapping is
  // unconfirmed. recompute()'s reference-list sweep raises the same fact
  // as a non-blocking rail warning once a lot's lot_ps_lab actually holds
  // one of these values - see there for why a flagged VALUE, not just a
  // flagged row sitting unused in the list, is what triggers it.
  producer_supplier_labs: {
    table: "producer_supplier_labs", select: "lab_id, amp_number, lab_name, flagged_mismatch", orderBy: "lab_name",
    noun: "producer/supplier labs",
    value: (r) => r.lab_id,
    label: (r) => `${r.lab_id} — ${r.lab_name} — ${r.amp_number}` +
...
const SUBLOT_TICKETS_SPEC = {
  ...
  //
  // IN THE LOT PDF the `fr` is what sizes the columns (gridWeights()): 1.4
  // is the smallest step that prints an ISO date whole in the PDF's 345pt
  // table - every date is the same 44.9pt there, and at 1fr each printed as
  // "2026-...". The total stays 10.6, so every PDF column not named here
  // keeps its width. The 1.4 comes from the three short number columns,
  // which still print a 5-digit truck number and a 3-digit tonnage or
  // temperature whole; what is left goes to AC method.
...
export const PLANTBOOK_SECTIONS = [
  {
    ...
    fields: [
      ...
      // mainline and 2 joint cores per sublot, 0.75 / 1.00 / 1.50 take 4
      // mainline and none, and a No. 4 takes no cores at all (Jake, same day:
      // "no. 4 mixes dont have cores") - so it is paid under Option B, which
      // densityOptionFor() seeds. Still a field rather than a readout, because the proposal's

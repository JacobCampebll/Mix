...
export default async (req) => {
  ...
  if (!payload || !approval || !approval.code || !approval.issued_at || !approval.approved_by)
  ...
  const valid = a.length === b.length && timingSafeEqual(a, b)
    && approval.approval_no === `KY-${String(approval.issued_at).slice(2, 4)}-${fp.slice(0, 6)}`;

  return valid
    ? json(200, { valid: true, approval_no: approval.approval_no, approved_by: approval.approved_by,

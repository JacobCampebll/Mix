...
import { signingMaterial, submitterOf, encodeDigest } from "./_canonical.mjs";
...
export default async (req) => {
  ...
  const { payload, submitted_action } = body || {};
  ...
  const digest = createHmac("sha256", secret)
    .update(signingMaterial({ payload, approvedBy: who.sm_id, submittedBy, issuedAt: issued_at }))
    .digest("hex");

  const fp = encodeDigest(digest, 18);
  return json(200, {
    approval_no: `KY-${issued_at.slice(2, 4)}-${fp.slice(0, 6)}`,
